...
Stacked two-panel, single-column (shared x-axis = clutter penalty κ):
  ...
  (top)  worst-user **min-SINR** vs κ  — the communication cost of clutter
         avoidance.  As κ grows the beamformer spends degrees of freedom
         steering a null toward the clutter PAS, so min-SINR drifts down.

  (bot)  **sum-SCNR** vs κ  — the sensing benefit.  As κ grows clutter
...
shipped system sits in the gentle-cost / near-saturated-gain region rather
than at an extreme.
...
joint_opt.py).  The ``kappa_sweep`` recipe therefore inherits the
``offset`` strategy from _defaults.sh; keep it that way for this figure.
...
Usage::
    ...
    python3 .../build_fig_clutter.py --kappa-star 0.08
    python3 .../build_fig_clutter.py --scnr-metric weighted_sum_scnr_db
...
# Sensing axis: prefer the sum-SCNR the sibling sweep figures use, falling
# back through weighted-sum / mean.  First metric the run actually has wins.
...
def collect_summary(result,
    ...
    suppression gain SCNR(κ_max)−SCNR(κ_min), and the min-SINR cost at κ\\*."""
    keys = sorted(result.sweep_results.keys())
...
def _print_summary(result, summary, scnr_metric, kappa_star) -> None:
    keys = sorted(result.sweep_results.keys())
    k_star = _nearest_kappa(result, kappa_star)
    ...
    for name, r in summary.items():
        ...
        def f(v, suf=""): return (f"{v:.2f}{suf}" if v is not None else "—")
        print(f"{name:<14}"
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    summary = collect_summary(result, only_resolved, scnr_metric, args.kappa_star)
    _print_summary(result, summary, scnr_metric, args.kappa_star)
