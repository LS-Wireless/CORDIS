...
def build_cdf_notebook() -> dict:
    cells = [
        ...
        md(
            "## 3. Alternate metrics",
            "",
            "The SimResult typically carries several metrics per run.  Common ones:",
            "`min_sinr_db`, `avg_sinr_db`, `sum_scnr_db`, `mean_scnr_db`.  Try them",
        ...
        code(
            ...
            "plot_cdf(result.sim_result, metric='avg_sinr_db', ax=axes[1], only=PICK)",
...
def build_sweep_notebook() -> dict:
    cells = [
        ...
        md("## 1. Quick sweep plot — one metric, all algorithms"),
        code(
            "from cordis.plotting import plot_sweep, figsize",
            "",
            "fig, ax = plt.subplots(figsize=figsize(width='single', aspect=3/2))",
            "plot_sweep(result.sweep_results, result.sweep_axis,",
            "           metric='min_sinr_db', ax=ax)",
        ...
        md(
            ...
            "`sweep_plot_pair` is a convenience for the two-panel layout.",
        ),
        code(
            "from _plot_common import sweep_plot_pair    # in scripts/, but importable here",
            "import sys; sys.path.insert(0, '../scripts')",
            "from _plot_common import sweep_plot_pair",
            "",
            "fig, axes = plt.subplots(1, 2, figsize=figsize(width='double', aspect=2.5/1.5))",
            "plot_sweep(result.sweep_results, result.sweep_axis,",
            "           metric='min_sinr_db', ax=axes[0])",
            "axes[0].set_title('min-SINR [dB]')",
            "plot_sweep(result.sweep_results, result.sweep_axis,",
            "           metric='mean_scnr_db', ax=axes[1])",
        ...
        code(
            ...
            "fig, ax = plt.subplots(figsize=figsize(width='single', aspect=3/2))",
            "plot_sweep(result.sweep_results, result.sweep_axis,",
            "           metric='min_sinr_db', ax=ax, only=PICK)",
            "plt.show()",
        ),
        md(
        ...
        code(
            "fig, ax = plt.subplots(figsize=figsize(width='single', aspect=3/2))",
            "plot_sweep(result.sweep_results, result.sweep_axis,",
            "           metric='mean_scnr_db', ax=ax, only=PICK)",
            "ax.set_yscale('log')",
...
def build_trace_notebook() -> dict:
    cells = [
        ...
        code(
            ...
            "",
            "fig, ax = plt.subplots(figsize=figsize(width='single', aspect=3/2))",
            "plot_admm_convergence(result.admm_result, ax=ax)",
            "ax.set_title('ADMM convergence (primal + dual residuals)')",
            "plt.show()",
        ),
        md(
