#!/usr/bin/env python3
"""
scripts/plot_csi_sweep.py
=========================

Render the paper figure for the 'csi_sweep' experiment.

Loads the most recent results/exp_csi_sweep/<ts>/ run (override with
``--exp-dir``) and writes figures into figures/exp_csi_sweep/.
...
# this script is run directly (e.g. `python3 scripts/plot_csi_sweep.py`).
...
def main() -> None:
    args   = build_plot_parser("csi_sweep").parse_args()
    logging.basicConfig(level=logging.INFO, format="%(levelname)-7s %(message)s")
    result = load_result("csi_sweep", args)
