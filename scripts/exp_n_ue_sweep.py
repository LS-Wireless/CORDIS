...
Run via the matching shell wrapper::
    ...
    bash scripts/exp_n_ue_sweep.sh

…or directly::

    python3 scripts/exp_n_ue_sweep.py --help
...
def main() -> None:
    parser = build_base_parser("n_ue_sweep")
    add_sweep_args(parser,
                          default_n_drops=20,
                          default_n_real=2)
    parser.add_argument("--n-ue-values", default='',
                        help="Comma- or space-separated UE counts (e.g. '2,4,6,8,10'). Empty → use registry default.")
    args = parser.parse_args()

    extra_kwargs = {
        "n_drops":        args.n_drops,
        "n_realizations": args.n_realizations,
        "n_ue_values": parse_value_list(args.n_ue_values, int) or None,
    }
    ...
    run_experiment("n_ue_sweep", args, extra_kwargs)
