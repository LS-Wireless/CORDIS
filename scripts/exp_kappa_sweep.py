...
Run via the matching shell wrapper::
    ...
    bash scripts/exp_kappa_sweep.sh

…or directly::

    python3 scripts/exp_kappa_sweep.py --help
...
def main() -> None:
    parser = build_base_parser("kappa_sweep")
    add_sweep_args(parser)
    parser.add_argument("--kappa-values", default='',
                        help="Comma- or space-separated κ values (e.g. '0,0.1,0.5,1.0,1.5,2.0'). Empty → use registry default.")
    args = parser.parse_args()

    extra_kwargs = {
        "kappa_values": parse_value_list(args.kappa_values, float) or None,
    }
    ...
    run_experiment("kappa_sweep", args, extra_kwargs,
                   fallback_n_drops=20,
                   fallback_n_real=2)
