#!/usr/bin/env python3
"""
scripts/plot_clutter_cnr_sweep.py
=================================

Render the paper figure for the 'clutter_cnr_sweep' experiment.

Loads the most recent results/exp_clutter_cnr_sweep/<ts>/ run (override with
``--exp-dir``) and writes figures into figures/exp_clutter_cnr_sweep/.
...
# this script is run directly (e.g. `python3 scripts/plot_clutter_cnr_sweep.py`).
...
def main() -> None:
    args   = build_plot_parser("clutter_cnr_sweep").parse_args()
    logging.basicConfig(level=logging.INFO, format="%(levelname)-7s %(message)s")
    result = load_result("clutter_cnr_sweep", args)
