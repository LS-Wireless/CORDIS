#!/usr/bin/env python3
"""
scripts/plot_kappa_sweep.py
===========================

Render the paper figure for the 'kappa_sweep' experiment.

Loads the most recent results/exp_kappa_sweep/<ts>/ run (override with
``--exp-dir``) and writes figures into figures/exp_kappa_sweep/.
...
# this script is run directly (e.g. `python3 scripts/plot_kappa_sweep.py`).
...
def main() -> None:
    args   = build_plot_parser("kappa_sweep").parse_args()
    logging.basicConfig(level=logging.INFO, format="%(levelname)-7s %(message)s")
    result = load_result("kappa_sweep", args)
