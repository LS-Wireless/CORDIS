...
# {summary}
#
# Wrapper around scripts/exp_{name}.py.  Every knob below is listed for
...
#
#     N_TRIALS=500 bash scripts/exp_{name}.sh
#     SEED=123 OUTPUT_ROOT=/scratch/results bash scripts/exp_{name}.sh
# =============================================================================
...
# writes to its own ``results/exp_<name>/array_<jobid>_task_<id>/`` so
...
#
#     python3 scripts/aggregate_array_batch.py {name} <jobid>
#
# Submit:
#
...
def _format_sweep_cli(sweep_cli: List) -> str:
    """Emit argparse.add_argument lines for sweep / experiment-specific flags.

    A default of ``"None"`` (or empty) in the metadata is rendered as
    ``default=None`` in argparse so the registry's own default applies
    when the user doesn't pass the flag.  String-kind flags retain
    their ``"''"`` empty-string default — ``parse_value_list('', ...)``
    handles that downstream and the result is filtered out before
    being forwarded to the registry.
    """
    if not sweep_cli:
        return ""

    def _is_registry_sentinel(default) -> bool:
        if default is None:
            return True
        if isinstance(default, str):
            return default.strip().strip("'\"") in ("", "None")
        return False

    lines = []
    for flag, kind, default, helptext in sweep_cli:
        if kind == "str":
            # Empty string is fine for str; parse_value_list('', ...) -> [].
            default_repr = "None" if (default is None) else default
            lines.append(
                f'    parser.add_argument("{flag}", default={default_repr},'
                f'\n                        help={helptext!r})'
            )
        elif kind == "int":
            default_repr = "None" if _is_registry_sentinel(default) else default
            lines.append(
                f'    parser.add_argument("{flag}", type=int, default={default_repr},'
                f'\n                        help={helptext!r})'
            )
        elif kind == "float":
            default_repr = "None" if _is_registry_sentinel(default) else default
            lines.append(
                f'    parser.add_argument("{flag}", type=float, default={default_repr},'
                f'\n                        help={helptext!r})'
            )
        else:
...
def _format_sweep_options_block(sweep_cli: List) -> str:
    """Shell snippet declaring each sweep-flag env var.

    Every sweep flag is exposed as an empty-by-default env var so the
    user can see what's available; an empty value falls through to the
    JSON config (for config-backed knobs) or the registry default
    (for experiment-specific knobs like convergence_trace's
    drop_seed/realization_seed/n_admm_max).  The original "default"
    metadata field is kept for argparse but never injected into the
    .sh — that's what made the .sh override config values silently.
    """
    if not sweep_cli:
        return ""
    lines = []
...
def _format_sweep_args_pack_block(sweep_cli: List) -> str:
    """Shell snippet conditionally packing sweep flags into SWEEP_ARGS.

    Mirrors the TRIAL_ARGS / RUN_ID_ARGS pattern: only add the flag to
    the python call if the user explicitly set the env var.  This keeps
    the config / registry as the single source of truth — an unset env
    var produces NO flag, which lets argparse's None default flow
    through to the registry where the real default lives.
    """
    if not sweep_cli:
        return ""
    lines = ["",
    ...
    for flag, _kind, _default, _help in sweep_cli:
        var = flag.lstrip("-").upper().replace("-", "_")
        lines.append(f'[ -n "${var}" ] && SWEEP_ARGS+=({flag} "${var}")')
    return "\n".join(lines)


def _format_sweep_args_block(sweep_cli: List) -> str:
    """Reference to ${{SWEEP_ARGS[@]}} inside the python call.

    The packing happens in _format_sweep_args_pack_block (placed before
    the python invocation).  Here we just emit the single array
    expansion that fans the conditionally-set flags into the call.
    """
    if not sweep_cli:
        return "    "
    return '    "${SWEEP_ARGS[@]}"             \\\n    '
...
def render_files(name: str, meta: Dict[str, Any]) -> Dict[Path, str]:
    ...
    out[REPO_ROOT / "scripts" / f"exp_{name}.sh"] = (
        RUNNER_SH_TEMPLATE.format(
            ...
            sweep_args_pack_block=_format_sweep_args_pack_block(meta["sweep_cli"]),
