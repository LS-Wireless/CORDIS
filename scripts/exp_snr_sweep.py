...
Run via the matching shell wrapper::
    ...
    bash scripts/exp_snr_sweep.sh

…or directly::

    python3 scripts/exp_snr_sweep.py --help
...
def main() -> None:
    parser = build_base_parser("snr_sweep")
    add_sweep_args(parser,
                          default_n_drops=20,
                          default_n_real=2)
    parser.add_argument("--snr-values-db", default='',
                        help='Comma- or space-separated SNR values in dB. Empty → use registry default.')
    args = parser.parse_args()

    extra_kwargs = {
        "n_drops":        args.n_drops,
        "n_realizations": args.n_realizations,
        "snr_values_db": parse_value_list(args.snr_values_db, float) or None,
    }
    ...
    run_experiment("snr_sweep", args, extra_kwargs)
