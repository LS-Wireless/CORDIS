#!/usr/bin/env python3
"""
scripts/plot_antennas_sweep.py
==============================

Render the paper figure for the 'antennas_sweep' experiment.

Loads the most recent results/exp_antennas_sweep/<ts>/ run (override with
``--exp-dir``) and writes figures into figures/exp_antennas_sweep/.
...
# this script is run directly (e.g. `python3 scripts/plot_antennas_sweep.py`).
...
def main() -> None:
    args   = build_plot_parser("antennas_sweep").parse_args()
    logging.basicConfig(level=logging.INFO, format="%(levelname)-7s %(message)s")
    result = load_result("antennas_sweep", args)
