#!/usr/bin/env python3
"""
scripts/plot_n_ue_sweep.py
==========================

Render the paper figure for the 'n_ue_sweep' experiment.

Loads the most recent results/exp_n_ue_sweep/<ts>/ run (override with
``--exp-dir``) and writes figures into figures/exp_n_ue_sweep/.
...
# this script is run directly (e.g. `python3 scripts/plot_n_ue_sweep.py`).
...
def main() -> None:
    args   = build_plot_parser("n_ue_sweep").parse_args()
    logging.basicConfig(level=logging.INFO, format="%(levelname)-7s %(message)s")
    result = load_result("n_ue_sweep", args)
